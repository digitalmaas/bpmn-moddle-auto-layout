  ...
  while (queue.length !== 0) {
      ...
      if (elementOrConnection.$type === 'bpmn:SubProcess') {
        ...
        if (elementOrConnection.flowElements) {
          ...
          const outgoing = getOutgoingConnection(elementOrConnection, children);
    ...
    if (incoming.length) {
      incoming.forEach(function(connection) {
        ...
        if (source.$type !== 'bpmn:SubProcess') {
          ...
          console.log('subprocessEnd', subprocessEnd)


          console.log('flowElement', flowElement)

          if (subprocessEnd.build_marked) {
            parent = self.tree.getNodeByName(subprocessEnd.id)
          } else {
            // parent = self._addTreeNode(subprocessEnd, flowElements)
          }
        }
        if (parent) {
