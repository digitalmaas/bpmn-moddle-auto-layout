...
AutoLayout.prototype._buildTreeBreadFirstSearch = function (rootFlowElement) {
  ...
  while (queue.length !== 0) {
      ...
      if (elementOrConnection.$type === 'bpmn:SubProcess') {
        const subprocessNode = self._addTreeNode(elementOrConnection, children)
        self._buildTreeBreadFirstSearch(elementOrConnection)
        if (elementOrConnection.flowElements) {

          // TODO ... add subprocess name to nodes
          // add outgoing node to end node of sub process

          const subprocessStartEvents = getStartEvents(elementOrConnection.flowElements)
          const subprocessStartNodes = subprocessStartEvents.map(startEvent => {
            return self.tree.getNodeByName(startEvent.id)
          })
          const subprocessEndEvents = getEndEvents(elementOrConnection.flowElements)
          ...
          const childNodes = [...subprocessNode.children]
  ...
  if (!flowElement.build_marked) {
    ...
    const oldParents = [...node.parents]
      ...
      let padParent = parent
        ...
        padParent = self.tree.add(this.nodeCount, PADDING_NODE, 100, 0)
        this.nodeCount += 1
...
AutoLayout.prototype._layoutConnections = function(group, parentDi) {
  ...
  connections.forEach(function(connection) {
    ...
    if (!directTarget && sourceNode.children && sourceNode.children.length > 0) {
    ...
    if (sourceNode.children && sourceNode.children.length > 0 && !directTarget) {
      ...
      if (DiFactoryUtils.is(sourceRef.$type, 'bpmn:Gateway')) {
        ...
        const first = { x: pathNodes[1].XPosition, y: pathNodes[1].YPosition }
      ...
      pathNodes.forEach(pathNode => {
        ...
        points.push({ x: pathNode.XPosition, y: pathNode.YPosition})
      })
...
function getPathToTarget(sourceNode, targetId) {
  ...
  if (sourceNode.children) {
    let pathToTarget
    sourceNode.children.forEach(function (source) {
...
function getStartEvents(flowElements) {
  return flowElements ? flowElements.filter(function(e) {
...
function getIncomingConnection(target, flowElements) {
  return flowElements ? flowElements.filter(function(c) {
    return DiFactoryUtils.is(c.$type, 'bpmn:SequenceFlow') && c.get('targetRef').id === target.id
  }) : []
}
