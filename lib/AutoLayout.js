...
AutoLayout.prototype._buildTreeBreadFirstSearch = function (rootFlowElement) {
  ...
  while (queue.length !== 0) {
      ...
      if (elementOrConnection.$type === 'bpmn:SubProcess') {
        ...
        const rootFlowElement = getStartEvent(elementOrConnection.flowElements)
  ...
  if (!flowElement.build_marked) {
    ...
    if (incoming.length) {
      incoming.forEach(function(connection) {
        ...
        let parent = target.build_marked ? self.tree.getNodeByName(target.id) : self._addTreeNode(target, flowElements)
        parentNode = parentNode || parent
        parentNodes.push(parent)
      });
    }
    parentNode = parentNode || { id: -1 }
    node = this.tree.add(this.nodeCount, parentNode.id, flowElement.id)
