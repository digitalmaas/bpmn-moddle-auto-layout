...
AutoLayout.prototype.layoutProcess = function(xmlStr, callback) {
  ...
  moddle.fromXML(xmlStr, function(error, moddleWithoutDi) {
    ...
    console.log('bpmn eleent layout complete')
...
AutoLayout.prototype._explodeSubprocess = function (flowElement, flowElements) {
  ...
  // then add the subprocess elements children to the subprocess end node
  /*
    Before (Bpmn):
      Process:
  ...
  */
  // the idea is that the subprocess node will be positioned correctly within the over all tree;
  ...
  // TODO ... I dont think we need the subprocess tree node!
