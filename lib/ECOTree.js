	...
	// console.log('l', l)
		...
		if (totalGap > 0) {
			...
			for (; subtreeAux != null && subtreeAux != leftAncestor; subtreeAux = subtreeAux._getLeftSibling()) {
				...
				// console.log('subtreeAux', subtreeAux)
			...
			if (subtreeAux != null) {
				...
				// console.log('subtreeMoveAux', subtreeMoveAux)
...
ECOTree._secondWalk = function (tree, node, level, X, Y, prevSiblings = []) {
	...
	if (level <= tree.config.iMaxDepth) {
		...
		if (node._getChildrenCount() != 0) {
			ECOTree._secondWalk(tree, node._getFirstChild(), level + 1, X + node.modifier, Y + maxsizeTmp + tree.config.iLevelSeparation, prevSiblings);
		}
