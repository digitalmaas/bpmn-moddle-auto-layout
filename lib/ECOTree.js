...
ECONode = function (id, pid, dsc, w, h) {
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
...
ECONode.prototype._getLevel = function () {
	if (this.nodeParents === undefined || this.nodeParents.length() === 0) {
		return 0
	} else {
		let maxLevel = 0
		this.nodeParents.forEach(nodeParent => {
			const nodeParentLevel = nodeParent._getLevel()
			maxLevel = (maxLevel < nodeParentLevel) ? nodeParentLevel : maxLevel
		})
		return maxLevel + 1
	}
}
...
ECONode.prototype._areParentsCollapsed = function () {
	if (this.nodeParents === undefined || this.nodeParents.length() === 0) {
		...
		this.nodeParents.forEach(nodeParent => {
			const nodeParentCollapsed = nodeParent._areParentsCollapsed()
			if (nodeParentCollapsed) {
				return nodeParentCollapsed
			}
		})
		return false
	}
...
ECOTree.prototype.add = function (id, pid, dsc, w, h) {
	...
	var node = new ECONode(id, pid, dsc, nw, nh);	//New node creation...
	node.nodeParent = pnode;  //Set it's parent
	node.nodeParents = [pnode]
