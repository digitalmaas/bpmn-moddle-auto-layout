...
DiFactory.prototype.is = function (type, expected) {
  const baseType = expected.split(':')[1]
  return type.indexOf(baseType) !== -1
}
...
DiFactory.prototype.createBpmnElementDi = function(elementType, attrs, pos) {
  ...
  if (elementType === 'connection') {
    ...
    let targetRef = connection.get('targetRef')
    let sourceRef = connection.get('sourceRef')
    var targetBounds = targetRef.bounds
    var sourceBounds = sourceRef.bounds;
    var preferredLayout = 'h:h'
    if (this.is(sourceRef.$type, 'bpmn:StartEvent') || this.is(targetRef.$type, 'bpmn:EndEvent')) {
      preferredLayout = 'h:v'
    }
...
DiFactory.prototype._getDefaultSize = function(element) {
  ...
  if (this.is(elementType, 'bpmn:SubProcess')) {
  ...
  if (this.is(elementType, 'bpmn:Task')) {
    return { width: 100, height: 80 };
  }

  if (this.is(elementType, 'bpmn:Gateway')) {
    return { width: 50, height: 50 };
  }

  if (this.is(elementType, 'bpmn:StartEvent') || this.is(elementType, 'bpmn:EndEvent')) {
    return { width: 36, height: 36 };
  }

  if (this.is(elementType, 'bpmn:Participant')) {
    if (element.isExpanded) {
      return getExpandedBounds(element)
    }
  ...
  if (this.is(elementType, 'bpmn:Lane')) {
    return { width: 400, height: 100 };
  }

  if (this.is(elementType, 'bpmn:DataObjectReference')) {
    return { width: 36, height: 50 };
  }

  if (this.is(elementType, 'bpmn:DataStoreReference')) {
    return { width: 50, height: 50 };
  }

  if (this.is(elementType, 'bpmn:TextAnnotation')) {
